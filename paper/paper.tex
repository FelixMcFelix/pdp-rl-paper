...
?? NOTE: this aggregastion step is specifically WAIT-FREE, not just lockless etc. because progress always made.
...
?? verify these numbers against code?

?? NOTE: Ritchie is actually Intel(R) Xeon(R) Silver 4208 CPU (8 times 2.1GHz)
...
\fakepara{Raw inference and learning performance}
\Cref{tab:lats} shows that \Coopfw{} \approachshort{} achieves sub-\SI{50}{\micro\second} median latency, with \nth{99} percentile latencies less than \SI{1}{\micro\second} worse.
...
\Cref{tab:tputs} shows how \approachshort{} compares in terms of throughput with a numpy-based implementation of an RL policy.
