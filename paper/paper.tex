...
\setlength{\dbltextfloatsep}{6.0pt plus 2.0pt minus 4.0pt}
\setlength{\dblfloatsep}{6.0pt plus 1.0pt minus 2.0pt}
...
\setlength{\resultplotwidth}{0.85\linewidth}
...
We chose the worker count on host machines that maximised their throughput.
...
\fakepara{Resource requirements}
\Cref{tab:resources} demonstrates how \approachshort{} consumes shared memory as it scales to fit a device's compute resources, compared with a simple P4 forwarding application.
...
We observe negligible costs on shared bulk EMEM storage ($\sim$\SI{4}{\mebi\byte}), incurred due to hashtables for state-action pairs and rewards.
