...
\begin{abstract}
Automatic, data-driven networking is becoming more capable and widely researched, driven by the efficacy of \emph{deep reinforcement learning} (DRL) algorithms.
...
Online learning and control have been lost, despite being useful techniques when problems evolve or are hard to model.

We present \emph{\approachshort{}}---\approach{}---to our knowledge the first work to bring \emph{online reinforcement learning} to the dataplane.
...
\approachshort{}'s designs scale with additional resources at compile-time to improve upon both decision latency and throughput, improving upon offloading using \emph{just one worker}, are quickly reconfigurable at runtime, and are task-agnostic.
...
Parallel to this, P4~\parencite{DBLP:journals/ccr/BosshartDGIMRSTVVW14} and NetFPGA devices~\parencite{DBLP:journals/micro/ZilbermanACM14} have inspired explosive growth and interest in the research community surrounding in-network computation and offloading.
...
In response, we have seen classical ML~\parencite{DBLP:conf/hotnets/XiongZ19} techniques, neural networks~\parencite{DBLP:conf/sigcomm/SanvitoSB18,DBLP:journals/corr/abs-1801-05731,DBLP:journals/corr/abs-2009-02353,langlet-ml-netronome}, and hardware design proposals for CGRA-based map-reduce blocks~\parencite{DBLP:journals/corr/abs-2002-08987} \emph{directly in the dataplane}.
...
For instance, \emph{deep neural network} (DNN) training relies upon the backpropagation algorithm to compute gradients, and for sizeable amounts of data to be held in mini-batches for stable training.
...
%?? Offloading costs
%?? High-throughput, low response latency to network changes.
%?? Mention PCIe offloading costs, DNN costs, potential IOMMU contention in virtualisation-based envs over NIC accesses and GPU accesses~\parencite{DBLP:conf/sigcomm/NeugebauerAZAL018}.

%?? Using classical RL? cost now is offloading latency
%?? Using DRL? Other costs.
%?? Deep RL training infamous for taking  
%
%Offloading to a host machine has several drawbacks; the additional latency
%
%?? In-network ML has focussed on inference, NOT online training...
%?? But they have innovations in data formats etc.
%Moreover, necessary hardware, such as floating-point units, is unavailable in almost all cases.
%
%?? P4 caused an explosion of suitable measurement techniques in plane...
%?? Classes of device exist today
%?? So... ripe for putting online learning agents here!
%
%?? So we do this...
%?? Classical RL techniques using the advances and observations of other datapath ML.
%?? path-adjacent, on-chip, in the dataplane (\cref{fig:netro-arch})
%?? Exploits legacy of many p4 devices to provide general purpose compute.
%
%?? Can I fit in the state-slippage shit here?
...
In particular, we exploit the (pre-P4) legacy of general purpose compute in SmartNIC devices to offer path-adjacent, on-chip RL in the dataplane (\cref{fig:netro-arch}).
...
\begin{itemize}
	...
	\item In-depth evaluation of how our approach affects carried dataplane traffic, performs under different policy sizes, and improves upon explicit offloading to produce a \SI{15}{$\times$} latency reduction compared to commodity hardware (\SI{21}{$\times$} for 99.99\nthscript{th} tail latencies) and an order of magnitude improvement in online throughput (\cref{sec:evaluation}).
...
We also contrast these against deep RL policies on network tasks, which can take \SI{3}{\milli\second} to compute~\parencite{DBLP:journals/corr/abs-1910-04054}---2 orders of magnitude above \approachshort{}.
...
These algorithms (and in-NIC deployment) enabled a \SIrange{15}{21}{$\times$} reduction in median--\num{99.99}\nthscript{th} inference times and order of magnitude improvement in online learning throughput.
