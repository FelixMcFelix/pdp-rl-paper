...
As anticipated, the \Coopfw{} strategy is key in achieving serviceable throughput in an online learning agent, \SI{9.9}{$\times$} a dedicated collector machine, due to the locking requirements mandating coordinated division of work.
...
This also explains why \SI{32}{\bit} becomes the best choice at different policy complexities for online (\cref{fig:vary-work}, 10 dims) and offline (\cref{fig:vary-work-latency}, 3 dims) \Coopfw{} agents, where hashtable accesses and a \texttt{memcpy} of the state vector fall into the serial portion of the algorithm.
...
\approachshort{} places load on the CPP bus through use of its IN/OUT rings for external communication and through last-tier policy accesses.
...
We expect that dedicated hardware/FPGA designs can avoid this problem by having dedicated In/Out access mechanisms for an \approachshort{} agent.
...
%			\includegraphics{../plots/build/stress/heat-latency-two-9-perc}
	...
	\end{subfigure}
	\caption{Effects on tail latency of cross-traffic caused by different loads of off-path RL compute. Statistically significant increases in population latency are concentrated on smaller packet sizes, and are typically sub-\SI{77}{\nano\second}.\label{fig:dataplane-coop}}
\end{figure}
...
As one program is installed per ME, these represent the minimum and maximum resource use we can command on a single island (i.e., without replacing P4 workers).
...
Thread local storage in CLS for compute/register spilling scales with the number of MEs required, particularly when caching  and with \Coopfw{} this is far below the ``fair share'' (allowing co-existence with other asynchronous dataplane programs).
...
These results show that \approachshort{} can be easily adapted and altered by network administrators once in place, and illustrates .
