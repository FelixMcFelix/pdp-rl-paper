...
These functions have lower model capacity, they do not require batches of inputs to learn in a stable way, negating the memory needed to store experience replays, and have simple update and inference logic.
...
\Cref{fig:netro-arch,fig:single-and-parallel} outline our design on Netronome SmartNIC hardware in pursuit of this goal: unused device resources beyond the P4-PSA spec \emph{can and should be used} to drive asynchronous environmental control.
...
\approachshort{} allows policy design and learning parameters to be changed at runtime using two control packets.
...
We show that when using 4 MEs of the NFP-6480, \Coopfw{} achieves sub-\SI{35}{\micro\second} median latency, with \nth{99} and \num{99.99}\nthscript{th} percentile latencies less than \SI{1}{\micro\second} worse (\SIlist{15;21}{$\times$} speedups over a \emph{Collector} host).
...
\num{99.99}\nthscript{th} percentile state-action latencies exceed the median by \SIlist{0.58;0.66}{\percent} for \Indfw{} and \Coopfw{}, while host-based tail latencies are at least \SI{40.53}{\percent} greater even when the parallel worker count is at or below the physical core count.
...
\begin{table}
\caption{NFP memory use due to \approachshort{} using \numlist{1;4} MEs (\SI{32}{\bit}). CLS and CTM are shared between all programs on the same island, while EMEM and IMEM are shared between all NFP programs on a NIC.\label{tab:resources}}
\resizebox{\linewidth}{!}{
\begin{tabular}{
		@{}c
