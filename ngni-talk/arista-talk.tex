...
\title{Revisiting the Classics: Online RL in the Programmable Dataplane \emph{(IEEE NOMS '22)}}
...
\footcitetext{DBLP:journals/corr/abs-2002-08987}
\end{frame}
...
\begin{frame}{Feasibility on other architectures?}
	\begin{itemize}[<+->]
		\item \textbf{Tofino etc.?} \alert{Unlikely}.
		\item \textbf{Taurus?\footcite{DBLP:journals/corr/abs-2002-08987}} Exec model \emph{almost} right, but \alert{read-only}.
		\item \textbf{FPGA?} Probably an effective match to this model + algorithm.
		\item Research gap: hardware architectures built to collate and apply batches of samples, gradient updates?
	\end{itemize}
\end{frame}
...
\begin{frame}[standout]
	...
	\begin{itemize}
		...
		\item \alert{Future work}: use-cases (AQM, DDoS prevention \& accuracy), NetFPGA, transfer learning.
	\end{itemize}
	\alert{Questions?}\\
	{
		\scriptsize
	...
	\begin{tikzpicture}[overlay, remember picture]
		%		\node[above right=0.8cm and 0.9cm of current page.south west] (esnet-logo) {\includegraphics[width=2.75cm]{netlab-trim}};
		%		\node[right=1cm of esnet-logo] {\adjincludegraphics[height=2cm,trim={0 {.4\height} 0 {.05\height}},clip]{uofg}};
		\node[above left=0.2cm and 0.8cm of current page.south east] {\includegraphics[width=2.75cm]{netlab-fulllogo}};
		\node[above right=0.35cm and 0.8cm of current page.south west] {\includegraphics[width=2.75cm]{uofg-white}};
	\end{tikzpicture}
\end{frame}
...
\begin{frame}{Latency---All \approachshort}
	\centering
	\resizebox{\linewidth}{!}{\expandableinput ../tables/build/latency}
\end{frame}

\begin{frame}{Throughput---All \approachshort}
	\centering
	\resizebox{\linewidth}{!}{\expandableinput ../tables/build/tput}
\end{frame}
