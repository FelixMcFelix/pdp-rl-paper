...
quant_widths = range(32)

def total_per_model(model):
	return len(quant_widths) * len(discounts) * len(algos) * len(host_ps) * len(traffic_types) * (1 if model < 2 else len(single_learners)) * len(maths) * len(topols)
...
if __name__ == "__main__":
	...
	quantw = expt_part(quant_widths, deps)
	...
	(rewards, good_traffic_percents, total_loads, store_sarsas, rng, action_comps, local_quant_results) = results

	file_name_part = "quant-train-{}".format(quantw)
	...
	lessDumbWriter(deltas_dir, action_comps)
