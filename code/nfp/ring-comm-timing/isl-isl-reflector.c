...
__declspec(import, addr40, emem) struct done_t isl_time_lock;

volatile __emem_n(1) __declspec(import, addr40, aligned(512*1024*sizeof(unsigned int))) uint32_t isl_time_workq[512*1024] = {0};
volatile __emem_n(1) __declspec(import, addr40, aligned(512*1024*sizeof(unsigned int))) uint32_t isl_reply_workq[512*1024] = {0};
...
main() {
	...
	while (!can_leave) {
		...
		can_leave = isl_time_lock.done;

		xfer = 0;
		mem_write_atomic(&xfer, (__addr40 void*)&(isl_time_lock.lock_val), sizeof(uint32_t));
	}

	r_addr = mem_ring_get_addr(isl_time_workq);
	reply_addr = mem_ring_get_addr(isl_reply_workq);
