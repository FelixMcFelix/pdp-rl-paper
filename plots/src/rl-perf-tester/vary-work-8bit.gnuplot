...
file(n) = sprintf("../results/rl-perf-tester/vary-work-ct/8/SUMMARY.%s.UpdateAll.dat", Names[(n / 2) + 1])

plot for [i=0:3] file(i) u 1:(column(TargetColumn[(i % 2) + 1])/1.2e3) with linespoints title myTitle(i) ls LineStyles[(i / 2) + 1] dt DashStyles[(i % 2) + 1]
