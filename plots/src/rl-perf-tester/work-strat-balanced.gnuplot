...
array PBitDepths[3]
PBitDepths[1] = "\\\qty{8}{\\\bit}"
PBitDepths[2] = "\\\qty{16}{\\\bit}"
PBitDepths[3] = "\\\qty{32}{\\\bit}"

#file(n) = sprintf("../results/rl-perf-tester/work-strats/32/%s.28d.31c.%s.dat", Names[(n / 3) + 1], Stages[(n % 3) + 1])
file(n) = sprintf("../results/rl-perf-tester/work-strats/%d/balanced.28d.31c.%s.dat", BitDepths[(n / 3) + 1], Stages[(n % 3) + 1])
x_coord(n) = 1 + (2*floor(n / 3)) + n

set xtics () # clear all tics
set xtics nomirror
set grid noxtics
set for [i=0:2] xtics add (PBitDepths[i + 1] x_coord(3*i)+1)

set key above

plot for [i=0:8] file(i) u (x_coord(i)):($1/1.2e3):(1.0) notitle ls LineStyles[(i % 3) + 2], \
